...
public abstract class BaseAccessibilityService extends AccessibilityService {
    ...
    public static AccessibilityNodeInfo findNodeInfoByClassName(AccessibilityNodeInfo nodeInfo, String className) {
        ...
        for (int i = 0; i < nodeInfo.getChildCount(); i++) {
            ...
            if(null != node ){
                MLog.d(TAG, "all className ("+i+") "+node.getClassName());
            }
            if (null != node && className.equals(node.getClassName())) {
                MLog.d("BaseQhbService", "xzhhbls...findNodeInfosByClassName.node=" + node.toString());
                return node;
            }
        }
     ...
     */
    private static AccessibilityNodeInfo findNodeInfoByClassNameRecycle(AccessibilityNodeInfo parentNodeInfo,
                                                                        String className) {
        if (null == parentNodeInfo || TextUtils.isEmpty(className)) {
            return null;
        }
        if (className.equals(parentNodeInfo.getClassName())) {
            MLog.d(TAG, "zyy...findNodeInfosByClassName.1.=" + parentNodeInfo.toString());
        ...
        for (int i = 0; i < parentNodeInfo.getChildCount(); i++) {
            ...
            if (null != nodeInfoResult) {
                MLog.d(TAG, "zyy...findNodeInfosByClassName.2.=" + nodeInfoResult.toString());
